...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_SWAGGER:
        case EFFECT_FLATTER:
            if (!AI_CanConfuse(battlerAtk, battlerDef, AI_DATA->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move, AI_DATA->partnerMove) && IS_BATTLER_OF_TYPE(battlerDef, TYPE_BUG))
                score -= 10;
            break;
