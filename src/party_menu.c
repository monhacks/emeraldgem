...
// {
    // if (TryTutorSelectedMon(species))
        // return TRUE;
    // else
        // return FALSE;
// }
...
// =======
    // u8 *strOut = ConvertIntToDecimalStringN(gStringVar1, &str, STR_CONV_MODE_RIGHT_ALIGN, 3);
...
static void CursorCb_HiddenMoves(u8 taskId)
{
	...
	if (CanLearnTeachableMove(GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_SPECIES), ITEM_HM02 - ITEM_TM01)) // If Mon can learn HM02 and action list consists of < 4 moves, add FLY to action list
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
    if (CanLearnTeachableMove(GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_SPECIES), ITEM_HM05 - ITEM_TM01)) // If Mon can learn HM05 and action list consists of < 4 moves, add FLASH to action list
...
{
    static const u8 askText[] = _("¿Querrías cambiar la habilidad de\n{STR_VAR_1} a {STR_VAR_2}?");
    static const u8 doneText[] = _("¡{STR_VAR_1} ahora tiene\n{STR_VAR_2}!{PAUSE_UNTIL_PRESS}");
    s16 *data = gTasks[taskId].data;
...
static void Task_LearnedMove(u8 taskId)
{
    ...
    GetMonNickname(mon, gStringVar1);
    StringCopy(gStringVar2, gLongMoveNames[move[0]]);
...
{
    StringCopy(gStringVar2, gLongMoveNames[gPartyMenu.data1]);
...
void CursorCb_MoveItemCallback(u8 taskId)
{
    ...
    case 1:     // User hit A on a Pokemon
		...
		// Pokemon can't swap items with a mail holder
