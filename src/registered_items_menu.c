...
static void TxRegItemsMenu_PrintFunc(u8 windowId, s32 id, u8 y);
...
static void TxRegItemsMenu_InitMenuFunctions(u8 taskId)
{
    ...
    TxRegItemsMenu_CountUsedRegisteredItemSlots();
    ...
    if (id != -2)
    {
        if (gTxRegItemsMenu->unk666 != 0xFF)
        {
            if (gTxRegItemsMenu->unk666 == (u8)id)
                TxRegItemsMenu_PrintSwappingCursor(y, 0, 0xFF);
            else
                TxRegItemsMenu_PrintSwappingCursor(y, 0xFF, 0xFF);
        }
    }
}
