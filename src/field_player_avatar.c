...
static const u16 sPlayerAvatarGfxIdsSkin2[][2] =
{
...
// static const u16 sPlayerAvatarGfxToStateFlagSkin2[2][5][2] =
// {
    // [MALE] =
    // {
		// {OBJ_EVENT_GFX_MAY_NORMAL,     PLAYER_AVATAR_FLAG_ON_FOOT},
...
// static const u16 sPlayerAvatarGfxToStateFlagSkin3[2][5][2] =
// {
    // [MALE] =
    // {
			// {OBJ_EVENT_GFX_BRENDAN_NORMAL,     PLAYER_AVATAR_FLAG_ON_FOOT},
...
static void PlayerNotOnBikeMoving(u8 direction, u16 heldKeys)
{
    ...
    else if (FlagGet(FLAG_SYS_DEXNAV_SEARCH) && !(heldKeys & B_BUTTON))
    {
        gPlayerAvatar.creeping = TRUE;
        PlayerWalkNormal(direction);
    }
...
{	
	// switch (VarGet(VAR_COLORSKIN))
	// {
	// case 0:
		// if (FlagGet(FLAG_BAGLESS))
			// return sPlayerAvatarGfxIds[state][gender];
		// else
			return sPlayerAvatarGfxIds[state][gender];
	// case 1:
		// if (FlagGet(FLAG_BAGLESS))
			// return sPlayerAvatarGfxIdsSkin2[state][1];
		// else
			// return sPlayerAvatarGfxIdsSkin2[state][gender];
		// break;
	// case 2:
		// if (FlagGet(FLAG_BAGLESS))
			// return sPlayerAvatarGfxIdsSkin2[state][1];
