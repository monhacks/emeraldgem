...
static int DoPokedexSearch(u8 dexMode, u8 order, u8 abcGroup, u8 bodyColor, u8 type1, u8 type2) // ,u16 move)
{
    ...
    if (move != 0xFFFF)
    {
        ...
        for (i = 0, resultsCount = 0; i < sPokedexView->pokemonListCount; i++)
        {
            ...
            //Tutor
            // if(CanTeachMove(species, tutorMoveId))
            // {
                // sPokedexView->pokedexList[resultsCount] = sPokedexView->pokedexList[i];
                // resultsCount++;
                // continue;
...
static bool8 CalculateMoves(void)
{
    ...
    // for (i=0; i < TUTOR_MOVE_COUNT; i++)
    // {
        // if (CanTeachMove(species, i)) //if (sTutorLearnsets[species] & (1 << i))
        // {
    ...
    for (i = 0; i < times; i++)
    {
        ...
        case EVO_BEAUTY:
            ...
            case EVO_ITEM_HOLD_NIGHT:
                ...
                break;
            case EVO_MOVE:
                StringCopy(gStringVar2, gLongMoveNames[gEvolutionTable[species][i].param]);
